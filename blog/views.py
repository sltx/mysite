...
def blog_list(request):
    blog_all_list = Blog.objects.all()  # 获取所有博客文章
    paginator = Paginator(blog_all_list, settings.EACH_PAGE_BLOGS_NUMBER)  # 分页
    page_num = request.GET.get('page', 1)  # 获取url的页面参数,当前页
    print(page_num)
    page_of_blogs = paginator.get_page(page_num)  # 获取想要显示那页的博客文章
    ...
    # 获取日期归档对应的博客数量
    blog_dates = Blog.objects.dates('created_time', 'month',
                                    )
    blog_dates_dict = {}
